...
import { syncNebulizacionToSheet } from "@/lib/sync-nebulizacion.functions";

export const Route = createFileRoute("/nebulizacion")({
  head: () => ({
    meta: [
      { title: "Sincronizador Nebulización" },
      {
        name: "description",
        content:
          "Sube archivos CSV o TXT de Nebulización y sincroniza a Google Sheets.",
      },
    ],
  }),
  component: NebulizacionPage,
});

type Result = Awaited<ReturnType<typeof syncNebulizacionToSheet>>;

function NebulizacionPage() {
  const sync = useServerFn(syncNebulizacionToSheet);
