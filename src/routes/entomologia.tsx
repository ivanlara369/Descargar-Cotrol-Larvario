...
import { syncEntomologiaToSheet } from "@/lib/sync-entomologia.functions";

export const Route = createFileRoute("/entomologia")({
  head: () => ({
    meta: [
      { title: "Sincronizador Entomología" },
      {
        name: "description",
        content:
          "Sube archivos CSV o TXT de Entomología y sincroniza a Google Sheets.",
      },
    ],
  }),
  component: EntomologiaPage,
});

type Result = Awaited<ReturnType<typeof syncEntomologiaToSheet>>;

function EntomologiaPage() {
  const sync = useServerFn(syncEntomologiaToSheet);
