...
export const Route = createFileRoute("/api/public/sync-intra")({
  server: {
    handlers: {
      ...
      POST: async ({ request }) => {
        try {
          ...
          const { syncIntra } = await import("@/lib/sync-intra-core.server");
          return json(await syncIntra(files));
        } catch (e) {
          const msg = e instanceof Error ? e.message : "Error desconocido";
          console.error("sync-intra failed:", msg);
