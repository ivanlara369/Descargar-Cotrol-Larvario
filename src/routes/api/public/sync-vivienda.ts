...
export const Route = createFileRoute("/api/public/sync-vivienda")({
  server: {
    handlers: {
      ...
      POST: async ({ request }) => {
        try {
          ...
          const { syncVivienda } = await import("@/lib/sync-vivienda-core.server");
          return json(await syncVivienda(files));
        } catch (e) {
          const msg = e instanceof Error ? e.message : "Error desconocido";
          console.error("sync-vivienda failed:", msg);
