...
export const Route = createFileRoute("/api/public/sync-larvario")({
  server: {
    handlers: {
      ...
      POST: async ({ request }) => {
        try {
          ...
          const files = (body.files || [])
            ...
            .map((f) => ({ name: String(f.name || "archivo"), content: f.content as string }));
          ...
          const { syncFiles } = await import("@/lib/sync-sheet-core.server");
          return json(await syncFiles(files));
        } catch (e) {
          const msg = e instanceof Error ? e.message : "Error desconocido";
          console.error("sync-larvario failed:", msg);
