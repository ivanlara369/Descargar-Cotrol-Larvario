...
export const Route = createFileRoute("/api/public/sync-entomologia")({
  server: {
    handlers: {
      ...
      POST: async ({ request }) => {
        try {
          ...
          const { syncEntomologia } = await import("@/lib/sync-entomologia-core.server");
          return json(await syncEntomologia(files));
        } catch (e) {
          const msg = e instanceof Error ? e.message : "Error desconocido";
          console.error("sync-entomologia failed:", msg);
