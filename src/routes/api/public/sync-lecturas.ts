...
export const Route = createFileRoute("/api/public/sync-lecturas")({
  server: {
    handlers: {
      ...
      POST: async ({ request }) => {
        try {
          ...
          const { syncLecturas } = await import("@/lib/sync-lecturas-core.server");
          return json(await syncLecturas(files));
        } catch (e) {
          const msg = e instanceof Error ? e.message : "Error desconocido";
          console.error("sync-lecturas failed:", msg);
