...
export const Route = createFileRoute("/api/public/sync-rociado")({
  server: {
    handlers: {
      ...
      POST: async ({ request }) => {
        try {
          ...
          const { syncRociado } = await import("@/lib/sync-rociado-core.server");
          return json(await syncRociado(files));
        } catch (e) {
          const msg = e instanceof Error ? e.message : "Error desconocido";
          console.error("sync-rociado failed:", msg);
