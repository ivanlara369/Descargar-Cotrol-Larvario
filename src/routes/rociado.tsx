...
import { syncRociadoToSheet } from "@/lib/sync-rociado.functions";

export const Route = createFileRoute("/rociado")({
  head: () => ({
    meta: [
      { title: "Sincronizador Rociado a Caso" },
      {
        name: "description",
        content:
          "Sube archivos CSV o TXT de Rociado a Caso y sincroniza a Google Sheets.",
      },
    ],
  }),
  component: RociadoPage,
});

type Result = Awaited<ReturnType<typeof syncRociadoToSheet>>;

function RociadoPage() {
  const sync = useServerFn(syncRociadoToSheet);
